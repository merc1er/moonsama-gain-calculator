...
var khaosSubgraphQueries = {
  ...
  all_rss_sama_pair_query: "query MyQuery($resources_addresses: [String!]) { tokens { pairQuote(where: {token0_in: $resources_addresses, token1: \"0x8c992cba48189a79204223d106fcb1d797a5f87a\"}) { token0 { id symbol } token1 { id symbol } token0Price token1Price } }}",
  rss_sama_pair_query: "query MyQuery($rss: String!) { tokens { pairQuote(where: {token0: $rss, token1: \"0x8c992cba48189a79204223d106fcb1d797a5f87a\"}) { token0 { id symbol } token1 { id symbol } token0Price token1Price } }}",
...
async function getUSDPriceOfRss() {
  ...
  let queryString = khaosSubgraphQueries.all_rss_sama_pair_query;
...
async function getPriceOfARssInSama(address) {
  console.log("Getting price of a rss in sama...");
  let url = 'https://graph.khaos.exchange/subgraphs/name/exosama-dex';
  let headers = {
    'Accept-Language': 'en-US,en;q=0.9,zh-CN;q=0.8,zh;q=0.7,it;q=0.6',
    'Connection': 'keep-alive',
  ...
  const response = await Promise.resolve(performQuery(url, variables, headers, queryString).catch(err => console.error(err)));
  let data = response.data.data.tokens;
  console.log(data)

  for (let i = 0; i < data.length; i++) {
    if (data[i].pairQuote.length != 0) {
      console.log(" DATA FOUND..." +  data[i])
      price_in_sama = data[i].pairQuote[0].token1Price;
      rss_name = data[i].pairQuote[0].token0.symbol;
    }
  }
  return price_in_sama;
}
...
// let q = "0x122EBE2B679cF54Bc8a6e89c1009714b354e2d10".toLowerCase()
// console.log(q)
// getPriceOfARssInSama(q)

export { getUSDPriceOfRss, getUSDPriceOfSama, getPriceOfARssInSama }; 