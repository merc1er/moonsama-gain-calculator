...
import { getUSDPriceOfRss, getUSDPriceOfSama, getPriceOfARssInSama } from './queryprices.js';
 ...
 * @property {number} khaosExchangePrice
 */
...
async function getAllMaterialsSama(){
  console.log("Get all materials sama...")
  const proms = []
  let rawResult = [];
  for(const material of materials){
    const prom = getMaterialSama(material.tokenId, material.address)
    proms.push(prom)
    prom.then(({lowestSell, highestBuy, khaosExchangePrice})=>{
      rawResult.push({tokenId: material.tokenId, name: material.name, address: material.address, lowestSell, highestBuy, khaosExchangePrice})
    })
  }
...
`query getAssetOrders {
  ...
  return {lowestSell, highestBuy, khaosExchangePrice}
}

/**
 * Returns the total USD value of all resources in SAMA
 * @param {Dict} resources the player gathered
 * @param {Dict} prices of resources in SAMA
...
async function getAllMaterialsMovr(){
  ...
  for(const material of materials){
    ...
    prom.then(({lowestSell, highestBuy})=>{
      rawResult.push({tokenId: matFerial.tokenId, name: material.name, lowestSell, highestBuy})
    })
  }
  await Promise.all(proms)
  rawResult = rawResult.sort((a, b) => a.tokenId - b.tokenId)
...
window.getAllMaterialsSama = getAllMaterialsSama
